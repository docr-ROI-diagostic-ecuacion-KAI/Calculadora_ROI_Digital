...
function compute(data: Data) {
  ...
  const normalized = Object.fromEntries(Object.entries(data.kai).map(([k, v]) => [k, v ? v / 5 : null]));
  const psi = normalized.data ?? null;
  const spo = normalized.customer && normalized.offer && normalized.nps ? normalized.customer * normalized.offer * normalized.nps : null;
  const kaiStar = [normalized.strategy, normalized.structure, normalized.operation, psi, spo, normalized.productivity, normalized.portfolio].every((v) => v !== null)
    ? [normalized.strategy, normalized.structure, normalized.operation, psi, spo, normalized.productivity, normalized.portfolio].reduce((a, b) => Number(a) * Number(b), 1)
    : null;
...
function Teaching({ title, children }: { title: string; children: React.ReactNode }) {
